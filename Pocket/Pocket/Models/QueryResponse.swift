...
public class QueryResponse: Codable {
    ...
    public required init(from decodable: Decoder) throws {
        ...
        result = try Utility.jsonStringToDictionary(string: stringData)
        
        let stringResult = try values.decodeIfPresent(String.self, forKey: .result) ?? ""
        result = try Utility.jsonStringToDictionary(string: stringResult)
    ...
    public func encode(to encoder: Encoder) throws {
        do {
            ...
            try container.encode(Utility.dictionaryToJsonString(dict: result), forKey: .result)
